...
def calculate_metrics(t, sf_raw, sl_raw, sr_raw, dt, sIdx, bIdx, zIdx, tIdx, lIdx):
    ...
    # Eccentric Braking RFD (Delta F / Delta t)
    ...
    # Time to Peak Force (ms) from movement start (sIdx)
    peak_force_idx = np.argmax(sf[sIdx:tIdx + 1]) + sIdx
    time_to_peak_force = (t[peak_force_idx] - t[sIdx]) * 1000.0

    # P1 / P2 Concentric Impulse (0-50ms and 50-100ms from concentric onset zIdx)
