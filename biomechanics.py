...
if t is not None and f_total is not None and len(f_total) > 0:
    ...
    # --- SESSION STATE INITIALIZATION FOR INTERACTIVE PHASE DRAGGING ---
    ...
    st.sidebar.markdown("---")
    st.sidebar.subheader("Phase Control (Drag lines on chart)")
    st.sidebar.info("💡 ท่านสามารถคลิกลากเส้นแนวตั้งบนกราฟเพื่อปรับแต่งเฟสได้โดยตรง (เฟสจะถูกล็อกลำดับอัตโนมัติ)")
    ...
    # Add draggable vertical lines using Plotly Shapes with `editable=True`
    fig_force.update_layout(
        title="FORCE-TIME ANALYSIS & SUB-PHASES (Click & Drag Vertical Lines)",
        xaxis_title="Time (s)",
        yaxis_title="Force (N)",
        height=450,
        shapes=[
    ...
    # Enable shape dragging in Plotly configuration
    config = {'editable': True, 'edits': {'shapePosition': True}}
    
    col_chart, _ = st.columns([1, 0.001])
    with col_chart:
        event = st.plotly_chart(fig_force, use_container_width=True, config=config, key="force_chart")

    # Capture drag events from Plotly chart and enforce strict sequential ordering constraints
