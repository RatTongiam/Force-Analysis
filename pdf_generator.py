...
def create_force_chart_image(t, sf, sl, sr, t_start, t_braking, t_split, t_takeoff, t_landing, crop_x_min=None, crop_x_max=None, is_coach=True):
    fig, ax = plt.subplots(figsize=(7.8, 2.7 if is_coach else 3.6), dpi=300)
    ...
    ax.set_title('Interactive vertical force–time curve', fontsize=9.5, fontweight='bold', color=col_tot, pad=4)
    ...
    for pic in pic_configs:
        ...
        if img is not None:
            imagebox = OffsetImage(img, zoom=0.10 if is_coach else 0.15)
...
def build_modern_coach_pdf(doc, report, t, sf, sl, sr, t_start, t_braking, t_split, t_takeoff, t_landing, threshold_alert, crop_x_min, crop_x_max, ctx):
    ...
    # 4. Page 1 Lower: Complete Braking & Propulsion Table (with All RFD Rows)
    bp_data = [
    ...
    # Row 4 (Page 2): Filter Sensitivity Analysis Table (Full Width 792 pt)
    sens_data = [
